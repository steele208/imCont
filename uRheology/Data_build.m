...
for im = 1 : length(imageInfo)
    ...
    for prtcl = 1 : size(imageInfo(im).trkInfo.Corners, 1)
                    ...
                    while numPIdx <= numel(prtclIdx)
                        ...
                        elseif ~any(particles(pID).Time == ...
                            ...
                            particles(pID).Time(end + 1,1) = ...
                                imageInfo(im).Meta.RelTime;
                            particles(pID).timeID(end + 1, 1) = ...
                                str2double(imageInfo(im).Meta.PlaneID);
                            added = 1;
                            break;
                        end
        ...
        if ~added % if flagged as un-associated, make new particle stem
            ...
            particles(end).timeID = str2double(imageInfo(im).Meta.PlaneID);
        end
    end
