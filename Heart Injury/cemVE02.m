...
% X & Y components of position of charges  [0, 0, 0, 0, 0]
    ...
    xC = [-(wellRad - space - a),  (wellRad - space - a),  ...
        (wellRad - space - a), -(wellRad - space - a), 0];
    yC = [ 0,    0,    0,    0,   0];
...
% GRAPHICS ===============================================================
% Restric plot to well region
xGplot = zeros(numel(xG),1);
yGplot = zeros(numel(xG),1);
for i = 1 : numel(xG)
    if(xG(i)^2 + yG(i)^2 > wellRad^2)
        xGplot(i) = NaN;
        yGplot(i) = NaN;
    else
        xGplot(i) = xG(i);
...
xGplot = reshape(xGplot, N, N);
yGplot = reshape(yGplot, N, N);
%% Figure 1
%{
...
figure(7)   % 7777777777777777777777777777777777777777777777777777777777
   ...
   pcolor(xG,yG,E./1e6);
   hold on;
   %draw circle depicting well plate boundary
   viscircles([0,0], wellRad); 
